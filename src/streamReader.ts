...
import { bufferTime, catchError, filter, firstValueFrom, Observable, take, timeout, TimeoutError, of } from "rxjs";
...
export class StreamReader {
  ...
  public async tryRead(maxCount: number, timeoutMs: number): Promise<StreamMessage[]> {
    const storedMessages = (await this.client.getNextMessages(this.streamId, {
    ...
    const liveMessagesStream = this.client.streamMessages(this.streamId, { latest: this.lastMessageId }).pipe(
    ...
    const firstMessages = await firstValueFrom(liveMessagesStream.pipe(catchError(err => {
